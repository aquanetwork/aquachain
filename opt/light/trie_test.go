...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb, _  = aquadb.NewMemDatabase()
		lightdb, _ = aquadb.NewMemDatabase()
		gspec      = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis    = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, aquahash.NewFullFaker(), vm.Config{})
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, aquahash.NewFaker(), fulldb, 4, testChainGen)
	...
	lightTrie, _ := NewStateDatabase(ctx, head, blockchain.Config().GetBlockVersion(head.Number), odr).OpenTrie(head.Root)
