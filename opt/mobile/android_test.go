...
import (
	...
	"github.com/aquanetwork/aquachain/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", AquaChain.LightScryptN, AquaChain.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.aquachain", new NodeConfig());
			node.start();
			...
			AquaChainClient ec = node.getAquaChainClient();
	...
	// signatures too. Regression test for aquachain issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "aquachain-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/aquachain/gethtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for AquaChain and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.aquachain", "github.com/aquanetwork/aquachain/opt/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "aquachain.aar"), "aquachain.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "aquachain", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.aquachain.gethtest"
...
dependencies {
    ...
    compile(name: "aquachain", ext: "aar")
}
`
