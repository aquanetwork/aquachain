...
import (
	...
	"gitlab.com/aquachain/aquachain/opt/les/flowcontrol"
...
func NewLesServer(aqua *aqua.AquaChain, config *aqua.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(aqua.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, aqua.EventMux(), aqua.Engine(), newPeerSet(), aqua.BlockChain(), aqua.TxPool(), aqua.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	srv.chtIndexer.Start(aqua.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(aqua.ChainDb())
	return srv, nil
}
...
func newCostStats(db aquadb.Database) *requestCostStats {
