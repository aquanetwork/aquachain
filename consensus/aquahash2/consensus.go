...
func (aquahash *Aquahash2) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
}

// VerifyHeader checks whether a header conforms to the consensus rules of the
// stock AquaChain aquahash engine.
func (aquahash *Aquahash2) VerifyHeader(chain consensus.ChainReader, header *types.Header, seal bool) error {
...
func (aquahash *Aquahash2) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
	// If we're running a full engine faking, accept any input as valid
...
func (aquahash *Aquahash2) verifyHeaderWorker(chain consensus.ChainReader, headers []*types.Header, seals []bool, index int) error {
...
func (aquahash *Aquahash2) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	// If we're running a full engine faking, accept any input as valid
	if aquahash.config.PowMode == ModeFullFake {
		return nil
	}
...
func (aquahash *Aquahash2) verifyHeader(chain consensus.ChainReader, header, parent *types.Header, uncle bool, seal bool) error {
...
func (aquahash *Aquahash2) CalcDifficulty(chain consensus.ChainReader, time uint64, parent *types.Header) *big.Int {
...
func (aquahash *Aquahash2) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
	...
	digest, result := hashingFunc(header.HashNoNonce().Bytes(), header.Nonce.Uint64())
	if !bytes.Equal(header.MixDigest[:], digest) {
		fmt.Printf("notequal digest.\nwant=%x\n got=%x\n", digest, header.MixDigest[:])
...
func (aquahash *Aquahash2) Prepare(chain consensus.ChainReader, header *types.Header) error {
...
func (aquahash *Aquahash2) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
