...
ADD aquastats.json /aquastats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../aqua-net-intelligence-api && pm2 start /aquastats.json)' >  explorer.sh && \
...
// explorerAquastats is the configuration file for the aquastats javascript client.
var explorerAquastats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.aquachain
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Aquastats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	aquastats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerAquastats)).Execute(aquastats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.aquastats[:strings.Index(config.aquastats, ":")],
		"Secret": config.aquastats[strings.Index(config.aquastats, ":")+1 : strings.Index(config.aquastats, "@")],
		"Host":   config.aquastats[strings.Index(config.aquastats, "@")+1:],
	})
	files[filepath.Join(workdir, "aquastats.json")] = aquastats.Bytes()
