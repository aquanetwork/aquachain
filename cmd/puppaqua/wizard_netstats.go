...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for aquastats availability")
	if infos, err := checkAquastats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["aquastats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["aquastats"] = infos.Report()
		aquastats = infos.config
	}
