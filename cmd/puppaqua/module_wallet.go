...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'aquad --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'aquad --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --aquastats \'{{.Aquastats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh
