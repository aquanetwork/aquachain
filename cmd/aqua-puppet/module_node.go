...
{{end}}
RUN \
  echo 'aquachain --cache 512 init /genesis.json' > aquachain.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.aquachain/keystore/ && cp /signer.json /root/.aquachain/keystore/' >> aquachain.sh && \{{end}}
	echo $'aquachain --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --aquastats \'{{.Aquastats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Aquabase}}--aquabase {{.Aquabase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> aquachain.sh

ENTRYPOINT ["/bin/sh", "aquachain.sh"]
`
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 aquachain --exec admin.nodeInfo.id attach", network, kind)); err != nil {
