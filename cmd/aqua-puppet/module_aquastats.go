...
// deployAquastats deploys a new aquastats container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployAquastats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	// Build and deploy the aquastats service
	if nocache {
...
// ethstatsInfos is returned from an aquastats status check to allow reporting
...
// checkAquastats does a health-check against an aquastats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkAquastats(client *sshClient, network string) (*ethstatsInfos, error) {
	// Inspect a possible aquastats container on the host
	...
	if err = checkPort(host, port); err != nil {
		log.Warn("Aquastats service seems unreachable", "server", host, "port", port, "err", err)
	}
