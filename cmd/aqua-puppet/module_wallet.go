...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'aquachain --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'aquachain --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --aquastats \'{{.Aquastats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh
