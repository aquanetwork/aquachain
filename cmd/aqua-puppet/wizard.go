...
type config struct {
	...
	aquastats string   // Aquastats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // AquaChain services known to be running on servers
...
// converts it to an AquaChain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by aquastats, which compares IPs textually, not by value.
