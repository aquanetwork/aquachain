...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"aquastats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "aquastats":
				if infos, err := checkAquastats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have aquastats running, ask whether to make the secret public or not
	if w.conf.aquastats != "" {
		fmt.Println()
		fmt.Println("Include aquastats secret on dashboard (y/n)? (default = yes)")
