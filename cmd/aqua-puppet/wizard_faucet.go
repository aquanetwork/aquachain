...
func (w *wizard) deployFaucet() {
	...
	if err != nil {
		infos = &faucetInfos{
			node:    &nodeInfos{port: 21303, peersTotal: 25},
			port:    80,
			host:    client.server,
			amount:  1,
			minutes: 1440,
	...
	fmt.Println()
	fmt.Printf("How many Aquaers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.aquastats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.aquastats = w.readString() + ":" + w.conf.aquastats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.aquastats)
		infos.node.aquastats = w.readDefaultString(infos.node.aquastats) + ":" + w.conf.aquastats
	}
