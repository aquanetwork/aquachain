...
import (
	...
	bzzapi "gitlab.com/aquachain/aquachain/opt/swarm/api"
)
...
var tomlSettings = toml.Config{
	...
	MissingField: func(rt reflect.Type, field string) error {
		...
		if unicode.IsUpper(rune(rt.Name()[0])) && rt.PkgPath() != "main" {
			link = fmt.Sprintf(", check gitlab.com/aquachain/aquachain/opt/swarm/api/config.go for available fields")
		}
...
func initSwarmNode(config *bzzapi.Config, stack *node.Node, ctx *cli.Context) {
	...
	//set the resolved config path (aquachain --datadir)
...
func envVarsOverride(currentConfig *bzzapi.Config) (config *bzzapi.Config) {
	...
	if datadir := os.Getenv(AQUAD_ENV_DATADIR); datadir != "" {
		currentConfig.Path = datadir
	}
...
func checkDeprecated(ctx *cli.Context) {
	// exit if the deprecated --aquaapi flag is set
	if ctx.GlobalString(DeprecatedAquaAPIFlag.Name) != "" {
		utils.Fatalf("--aquaapi is no longer a valid command line flag, please use --ens-api and/or --swap-api.")
	}
