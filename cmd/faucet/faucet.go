...
// faucet is a Aquaer faucet backed by a light client.
package main
...
var (
	...
	ethPortFlag = flag.Int("ethport", 21303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the AquaChain protocol")
	statsFlag   = flag.String("aquastats", "", "Aquastats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Aquaers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for AquaChain and the faucet")
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Aquaers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // AquaChain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an AquaChain light client.
type faucet struct {
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the AquaChain light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := aqua.DefaultConfig
	...
	// Assemble the aquastats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightAquaChain
			ctx.Service(&serv)
			return aquastats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := aquaclient.NewClient(api)
...
// close terminates the AquaChain connection and tears down the faucet.
...
// apiHandler handles requests for Aquaer grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the AquaChain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/aquanetwork/aquachain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and AquaChain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for AquaChain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No AquaChain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the AquaChain address and profile URL.
...
// returning the username, avatar URL and AquaChain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain AquaChain address,
